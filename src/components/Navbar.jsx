...
import { FaHome, FaInfoCircle, FaCogs, FaHandHoldingHeart, FaUsers, FaCommentDots } from "react-icons/fa";
...
export default function Navbar() {
  ...
  const handleLogoClick = () => {
    if (window.location.hash === "#hero" || window.scrollY < 100) window.location.reload();
  ...
  return (
    <>
      <nav
        ...
        style={{ backgroundColor: scrolled ? "rgba(255,255,255,0.35)" : "rgba(255,255,255,0.25)" }}
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12 flex justify-between items-center h-16 sm:h-20 md:h-24">
                ...
                return (
                  <a
                    ...
                    className={`relative flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300
                      ${isActive ? "bg-[#FFDCA0]/60 shadow-lg" : "hover:bg-[#FFDCA0]/30"}
                      hover:scale-110`}
                    title={label}
                    style={{ color: "#2D5D46" }}
                  >
                    <motion.div
                      whileHover={{ scale: 1.2, rotate: 10, textShadow: "0px 0px 8px rgba(255,215,100,0.8)" }}
                      className="text-lg"
                    >
                      {icon}
                    </motion.div>
          ...
          {/* Mobile Hamburger */}
          {isMobileView && (
            <button onClick={() => setOpen(!open)} className="text-3xl sm:text-4xl" style={{ color: "#2D5D46" }} aria-label="Toggle Menu">
              {open ? "✕" : "☰"}
            </button>
          )}
        </div>
        ...
        <AnimatePresence>
          {open && isMobileView && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="border-t rounded-b-3xl backdrop-blur-xl shadow-lg transition-all duration-300"
              style={{ backgroundColor: "rgba(255,255,255,0.25)", borderColor: "rgba(255,255,255,0.2)" }}
            >
              <div className="flex flex-col px-4 sm:px-6 py-4 font-semibold gap-1">
                {links.map(({ label, id }, i) => {
                  ...
                  return (
                    <motion.a
                      ...
                      className="px-3 py-2 rounded-lg text-center transition-all duration-300 hover:bg-white/20 hover:scale-105"
                      style={{
                        backgroundColor: isActive ? "rgba(255,237,214,0.6)" : "transparent",
                ...
                <motion.button
                  ...
                  className="px-3 py-2 rounded-lg font-semibold text-center transition-all duration-300 hover:bg-white/20 hover:scale-105 mt-1"
                  style={{
                    backgroundColor: "rgba(255,237,214,0.6)",
                    color: "#2D5D46",
                    fontSize: "clamp(14px, 2vw, 18px)",
                    backdropFilter: "blur(6px)",
                  }}
